    ...
    for (const apiKey of apiKeys) {
      try {
        ...
        if (encryptionUtil.isEncrypted(apiKey.key)) {
          console.log(`⏭️  API key ${apiKey._id} (${apiKey.name}) is already encrypted`);
        ...
        console.log(`✅ Encrypted API key ${apiKey._id} (${apiKey.name}) - ${originalKeyPrefix}`);
        encryptedCount++;

      } catch (error) {
        console.error(`❌ Failed to encrypt API key ${apiKey._id} (${apiKey.name}):`, error.message);
        errorCount++;
      }
    }
...
async function testEncryption() {
  try {
    ...
    console.log(`✅ Decryption successful: ${decrypted === testKey ? 'MATCH' : 'MISMATCH'}`);
    ...
    for (const apiKey of apiKeys) {
      try {
        ...
        if (decryptedKey && decryptedKey.length > 0) {
          verifiedCount++;
          console.log(`✅ API key ${apiKey._id} (${apiKey.name}) - decryption verified`);
        } else {
          throw new Error('Decrypted key is empty');
        }
      } catch (error) {
        console.error(`❌ Failed to decrypt API key ${apiKey._id} (${apiKey.name}):`, error.message);
        errorCount++;
      }
    }
...
`);
  ...
  if (!testPassed) {
    console.error('❌ Encryption test failed. Please check your ENCRYPTION_KEY environment variable.');
    process.exit(1);
  }

  if (args.includes('--test-only')) {
    console.log('✅ Test completed successfully. Encryption is working correctly.');
    process.exit(0);
  }
