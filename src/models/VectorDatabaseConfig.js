...
const vectorDatabaseConfigSchema = new mongoose.Schema({
  ...
  config: {
    ...
    weaviate: {
      endpoint: {
        type: String,
        required: function() { return this.provider === 'weaviate'; }
      },
      apiKey: {
        type: String,
        required: function() { return this.provider === 'weaviate' && this.config.weaviate?.requiresAuth; }
      },
      scheme: {
        type: String,
        enum: ['http', 'https'],
    ...
    pinecone: {
      apiKey: {
        type: String,
        required: function() { return this.provider === 'pinecone'; }
      },
      environment: {
        type: String,
        required: function() { return this.provider === 'pinecone'; }
...
vectorDatabaseConfigSchema.index({ organization_id: 1, project_id: 1, is_default: 1 });
vectorDatabaseConfigSchema.index({ organization_id: 1, project_id: 1, is_active: 1 });
...
vectorDatabaseConfigSchema.methods.updateUsageStats = function(operation, metrics = {}) {
  const now = new Date();
    ...
    case 'search':
      ...
      if (metrics.search_time_ms) {
        ...
        this.performance_metrics.avg_search_time_ms = 
          (currentAvg * (totalSearches - 1) + metrics.search_time_ms) / totalSearches;
      }
      break;
  }
  ...
  if (metrics.success !== undefined) {
    const totalOps = this.usage_stats.total_searches + this.usage_stats.total_documents;
...
// Static methods
vectorDatabaseConfigSchema.statics.findDefault = function(organizationId, projectId) {
  return this.findOne({
...
vectorDatabaseConfigSchema.statics.findByProject = function(organizationId, projectId, activeOnly = true) {
  const query = {
    organization_id: organizationId,
    project_id: projectId
  };
...
vectorDatabaseConfigSchema.statics.setDefault = async function(configId, organizationId, projectId) {
...
vectorDatabaseConfigSchema.pre('save', function(next) {
  ...
  if (this.provider === 'weaviate') {
    ...
    if (!this.config.pinecone?.apiKey || !this.config.pinecone?.environment || !this.config.pinecone?.indexName) {
      return next(new Error('Pinecone apiKey, environment, and indexName are required'));
    }
  }
...
module.exports = mongoose.model('VectorDatabaseConfig', vectorDatabaseConfigSchema);
